...
#slide[
  ...
  #align(center)[#text(size: 9pt)[*Item4 达标*（本机 RTX 3060 Laptop 原生实测，fp16 batch=4，纯前向）：104/106（S 档 9.7M）\@1024 = 32.1/31.7 FPS > 30，且 R 均超 RT-DETR 基线 0.757；103（L 档）0.775 为精度上限，17.4 FPS 需工程加速]]
]
...
#slide[
  ...
  #text(size: 9pt)[
    ...
    *三个关键读数*：①103（L）= 精度上限；②106（S，原图-only）= 消融最优，反超 104（S，混合池）；
    ③104/105/106（S）= 速度达标交付版（1024 下 32.1/29.7/31.7 FPS），精度仍超 RT-DETR 基线。
  ]
]
...
#slide[
  ...
  #align(center)[#image("assets/fps_accuracy_scatter.png", width: 97%)]
  #text(size: 9pt)[
    *左（\@1024，Item4 分辨率）*：S 档三个模型（104/106/105）在 30 FPS 线上方，L 档 103（17.4）与 096（27.4）在其下；
    *右（\@1280，精度最优）*：全部模型落到 30 FPS 线下——*分辨率与速度的取舍必须显式量化，不能靠折算*。
    #v(0.2em)
